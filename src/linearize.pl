...
%! \title Linearize constraints
%
%  \module
%
%    Obtain the linear approximation of a list of constraints.
%
% TODO: Document
...
%! linear_constraint(C): `C` is a linear constraint
...
linear_term(X-Y) :- 
	...
	linear_term(Y).
linear_term(X*Y) :- 
	( const(X), linear_term(Y)
	; const(Y), linear_term(X)
	).
linear_term(X/Y) :- 
	const(Y),
	linear_term(X).
